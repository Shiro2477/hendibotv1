        ...
        */
        ...
        switch (command) {
            ...
            case 'fb':
                ...
                .then(async ({ result }) => {
                            const { source } = result
                            const text = `➸ *...:* *${source} Downloader* *:...*\n`
                            await bocchi.sendFileFromUrl(from, result.VideoUrl, 'fbdl.mp4', text, id)
                    console.log('Success sending facebbok video result!')
                })
                .catch(async (err) => {
                    console.error(err)
                    await bocchi.reply(from, 'Error!', id)
            ...
            case 'ytmp3':
                ...
                downloader.ytdl(url)
                    .then(async (res) => {
                       ...
                       // } else if (Number(res[0].size(' MB')[0]) >= 50) {
    ...
    _keybot_ : ${prefix}jadwaltv tvri
        ...
        _*List TV Index*_`, id)
            ...
            case 'quotesnime':
                ...
                const quoteznime = await axios.get(`https://api.i-tech.id/anim/quotes?key=${config.itech}`)
                await bocchi.sendText(from, `➸ *Quotes* : ${quoteznime.data.data.quotes}\n➸ *Character* : ${quoteznime.data.data.character} from Anime ${quoteznime.data.data.anime}`, id)
